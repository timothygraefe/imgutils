...
def resize_image(img, basefile, iheight, iwidth, oheight, owidth, newdpi, commit=False, verbose=False):

    # The size of the base image should be larger than the output size
    print('Input image size: width: {} height: {}'.format(iwidth, iheight))
...
def main():
    parser = argparse.ArgumentParser(description='PNG Image Resize')
    ...
    parser.add_argument('-H', metavar='height', nargs=1, type=int, help='Output file height')
    parser.add_argument('-W', metavar='width', nargs=1, type=int, help='Output file width')
    parser.add_argument('-D', metavar='DPI', nargs=1, type=int, help='Output file DPI')
    parser.add_argument('-Q', metavar='quality', nargs=1, type=int, help='Output file quality')
    ...
    iwidth  = img.size[0]
    iheight = img.size[1]
    print('  Height: {} Width: {}'.format(iheight, iwidth))
    ...
    if((args.H == None) and (args.W == None)):
        print('You must specify a height or width')
        exit()
    ...
    if(args.H != None):
        ...
        if(args.W == None):
            scale = float(oheight / iheight)
            owidth = int( scale * float(iwidth) )
            print('Scaled output file width: {}'.format(owidth))
        else:
            owidth = args.W[0]
    else:
        owidth = args.W[0]
        scale = float(owidth / iwidth)
        oheight = int( scale * float(iheight) )
        print('Scaled output file height: {}'.format(oheight))
...
#   else:
#       owidth = iheight
#       print('Using input file for width: {}'.format(owidth))
    ...
    resize_image(img, basefile, iwidth, iheight, owidth, oheight, dpi, commit, verbose)
